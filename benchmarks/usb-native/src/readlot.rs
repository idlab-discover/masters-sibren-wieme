...
fn main() -> Result<()> {
    ...
    // 1) Initialize libusb/context and find the target device
    ...
    // Calculate byte‐range of the exFAT partition
    let slice_start = data_partition.starting_lba as u64 * block_len as u64;
    let slice_end =
        (data_partition.starting_lba as u64 + data_partition.sectors as u64) * block_len as u64;
    let slice_len = (slice_end - slice_start) as usize;

    // Build a single IoSlice over [slice_start .. slice_end)
    let slice = IoSlice::new(&mut usb, slice_start, slice_end)
        .context("Failed to create IoSlice")?;
    let mut reader = io::BufReader::new(slice);

    // Open an output file and stream‐copy the entire partition‐slice into it:
    let mut out_file = File::create("partition_dump.bin")
        .context("Failed to create partition_dump.bin")?;
    let bytes_written = io::copy(&mut reader, &mut out_file)
        .context("Failed to copy partition into file")?;
